...
def buton_olustur(
    ...
    if link:
        gidecek_buton: list = [
            InlineKeyboardButton(text=key, url=value) for key, value in butonlar.items()
        ]
    ...
    if data:
        gidecek_buton: list = [
            ...
            for key, value in butonlar.items()
        ]
        gidecek_buton = dilimleyici(gidecek_buton, adet)

    if ana_sayfa:
        geri_butonu: InlineKeyboardButton = InlineKeyboardButton(
            text="🏘 Ana Sayfa", callback_data=ana_sayfa
        )
        gidecek_buton.append([geri_butonu])

    if geri:
        geri_butonu: InlineKeyboardButton = InlineKeyboardButton(
            text="🔙 Geri", callback_data=geri
        )
        gidecek_buton.append([geri_butonu])
